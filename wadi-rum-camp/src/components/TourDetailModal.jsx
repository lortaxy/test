export default function TourDetailModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/80 z-50 overflow-y-auto" onClick={() => setSelectedTour(null)}>
      <div className="min-h-screen px-4 py-4 sm:py-8 flex items-start sm:items-center justify-center">
        <div className="bg-white rounded-xl sm:rounded-2xl max-w-5xl w-full" onClick={(e) => e.stopPropagation()}>
          <div className="relative">
            <img
              src={selectedTour.id === 1 ? `${import.meta.env.BASE_URL}WhatsApp Image 2026-01-29 at 15.24.14 (1).jpeg` : gallery[selectedTour.id]}
              alt={displayName}
              loading="lazy"
              className="w-full h-40 sm:h-48 md:h-56 object-cover rounded-t-xl sm:rounded-t-2xl"
            />
          <button
        ...
        <div className="p-4 sm:p-6 md:p-8">
          <div className="flex flex-col gap-3 sm:gap-4 mb-4 sm:mb-6">
            <div>
              <h2 className="text-xl sm:text-2xl md:text-3xl font-light text-amber-900">{displayName}</h2>
              <p className="text-sm sm:text-base text-amber-700 font-light">{displayDuration}</p>
            </div>
          ...
          <div className="grid lg:grid-cols-2 gap-4 sm:gap-6 mb-4 sm:mb-6">
            <div className="space-y-3 sm:space-y-4">
              <p className="text-sm sm:text-base text-amber-800 leading-relaxed font-light">{displayDesc}</p>

              {displayPricing && (
                <div className="bg-amber-50 border border-amber-200 rounded-lg sm:rounded-xl p-3 sm:p-4">
                  <h3 className="text-base sm:text-lg font-light text-amber-900 mb-2 sm:mb-3">{t.pricing}</h3>
                  <div className="space-y-1.5 sm:space-y-2 mb-2 sm:mb-3">
                    {displayPricing.map((tier, i) => (
                      <div key={i} className="flex justify-between items-center">
                        <span className="text-xs sm:text-sm text-amber-800 font-light">{tier.guests}</span>
                        <span className="text-sm sm:text-base text-amber-900 font-medium">{tier.price} JOD {t.perPerson}</span>
                      </div>
                    ))}
                  </div>
                  {displayPrivateNote && (
            ...
            <div className="space-y-3 sm:space-y-4">
              <div className="grid grid-cols-1 gap-3 sm:gap-4">
                <div className="bg-green-50 border border-green-200 rounded-lg p-2.5 sm:p-3">
                  <h3 className="text-xs sm:text-sm font-medium text-amber-900 mb-1.5 sm:mb-2">{t.includes}</h3>
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-2.5 sm:p-3">
                  <h3 className="text-xs sm:text-sm font-medium text-amber-900 mb-1.5 sm:mb-2">{t.highlights}</h3>
                ...
                {displayExcluded && (
                  <div className="bg-red-50 border border-red-200 rounded-lg p-2.5 sm:p-3">
                    <h3 className="text-xs sm:text-sm font-medium text-amber-900 mb-1.5 sm:mb-2">{t.excludes}</h3>
                    <ul className="space-y-0.5 sm:space-y-1">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 border-t border-amber-100 pt-4">
            {displayPricing ? (
              <p className="text-sm sm:text-base font-light text-amber-700">
              ...
              <p className="text-xl sm:text-2xl font-light text-amber-900">
                {selectedTour.price} JOD <span className="text-sm sm:text-base font-light text-amber-700">{t.perPerson}</span>
              </p>
            )}
            <a
              href="#booking"
